...
def OPF_model_creator_without_switch(np,pyo,base_MVA,Slackbus,Bus_info,Line_info,Load_info,Gen_info,Time_info):
    ...
    model.Loads = pyo.Set(dimen=2)
    model.Gens = pyo.Set(dimen=2)
    ...
    def P_demand_rule(model, i, t):
        return sum(
            Load_info.loc[d, 'p_mw'] / base_MVA
            for d in model.Loads
            if Load_info.loc[d, 'bus'] == i and d[1] == t
        )
    model.PDem = pyo.Expression(model.Buses, model.Times, rule=P_demand_rule)

    def Q_demand_rule(model, i, t):
        return sum(
            Load_info.loc[d, 'q_mvar'] / base_MVA
            for d in model.Loads
            if Load_info.loc[d, 'bus'] == i and d[1] == t
        )
    ...
    def P_gen_ini_rule(model, n, i, t):
        # n: (Gen_n, Time) 튜플
        if Gen_info.loc[n, 'bus'] == i and n[1] == t:
            return Gen_info.loc[n, 'p_mw'] / base_MVA
        else:
            return 0.0

    model.PGen = pyo.Var(model.Gens, model.Buses, model.Times, within=pyo.NonNegativeReals, initialize=P_gen_ini_rule)
    ...
    def P_line_flow_sending_rule(model, l, t):
        ...
        return ((-1) * model.Bus_G[i, j] * model.V_mag[i, t] * model.V_mag[i, t]
    ...
    def Q_line_flow_sending_rule(model, l, t):
        i = Line_info.loc[l, 'from_bus']
        j = Line_info.loc[l, 'to_bus']
        return (model.Bus_B[i, j] * model.V_mag[i, t] * model.V_mag[i, t]
    ...
    def P_gen_min_rule(model, n, i, t):
        if Gen_info.loc[n, 'bus'] == i and Gen_info.loc[n, 'Time'] == t:
