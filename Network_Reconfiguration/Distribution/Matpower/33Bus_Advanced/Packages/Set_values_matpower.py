...
def Set_Bus(pd,save_directory,mpc):
    ...
    # For information of transfer bus
    Bus_info['Transfer_Bus'] = 0
    ...
    filtered_gen_df = gen_info[gen_info['in_service'] == 1]
    ...
    filtered_load_df = load_info[(load_info['p_mw'] > 0.0) & (load_info['q_mvar'] > 0.0)]
    
    for bus in Bus_info['Buses']:
        if (bus in list(filtered_gen_df['bus'])) | (bus in list(filtered_load_df['bus'])): # Bus가 Generator bus 이거나 Load bus 라면
            Bus_info['Transfer_Bus'] = 0
        else:
            Bus_info['Transfer_Bus'] = 1
...
def Set_Line(pd,save_directory,m,mpc,previous_branch_array):
    ...
    Line_column = ['from_bus','to_bus','r_pu','x_pu','b_pu','rate_MVA','in_service','in_service (initial)']
